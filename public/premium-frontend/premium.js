...
displayContainer.addEventListener('click', (e) => {
    if (e.target.id == 'dltbtn') {
        ...
        axios.post(`http://54.176.162.231:3000/deleteExpense/${trId}`, {}, { headers: { 'authorization': `Bearer ${localStorage.getItem('token')}` } })
            .then((res) => {
                tr.remove()
                notifyUser(res.data.message)
            })
...
downloadBtn.addEventListener('click', () => {
    axios.get('http://54.176.162.231:3000/download', { headers: { 'authorization': `Bearer ${localStorage.getItem('token')}` } })
        .then(response => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    axios.get('http://54.176.162.231:3000/previousdownloads', { headers: { 'authorization': `Bearer ${localStorage.getItem('token')}` } })
        .then(response => {
