...
form.addEventListener('submit', (e) => {
    ...
    description.value = '';
    category.value = '';
    axios.post('http://54.176.162.231:3000/addExpense', obj, { headers: { 'authorization': `Bearer ${localStorage.getItem('token')}` } })
        .then(res => {
...
function displayExpenses(limit, page = 1, rows = localStorage.getItem('rows')) {
    ...
    axios.get(`http://54.176.162.231:3000/getExpenses?limit=${limit}&page=${page}&rows=${rows}`, { headers: { 'authorization': `Bearer ${localStorage.getItem('token')}` } })
        .then(response => {
...
displayContainer.addEventListener('click', (e) => {
    if (e.target.id == 'dltbtn') {
        ...
        //li.remove()
        axios.post(`http://54.176.162.231:3000/deleteExpense/${liId}`, {}, { headers: { 'authorization': `Bearer ${localStorage.getItem('token')}` } })
            .then((res) => {
                li.remove()
                notifyUser(res.data.message)
            })
...
premiumBtn.addEventListener("click", () => {
    ...
    axios.post('http://54.176.162.231:3000/premium/create/order', { amount: amount }, { headers: { 'authorization': `Bearer ${localStorage.getItem('token')}` } })
        .then(response => {
...
payBtn.addEventListener('click', (e) => {
    ...
    payBtn.style = "display:none"
    var options = {
        "key": "rzp_test_Xr9DKpctIwQxWu", // Enter the Key ID generated from the Dashboard
        ...
        "handler": function (response) {
            ...
            axios.post('http://54.176.162.231:3000/transaction/detail', { orderId: orderId, paymentId: paymentId }, { headers: { 'authorization': `Bearer ${localStorage.getItem('token')}` } })
                .then()
                .catch(err => {
                    console.log(err)
                })
